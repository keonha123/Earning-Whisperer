...
export default function TradingRoomPage() {
  ...
  //  - DEV fixture 시연 모드(demoActive): 실데이터가 없으므로 LIVE 강제 활성.
  //  - fallback: 그 외에는 신호 기반 판정. 단 현재 종목의 신호일 때만 —
  //    다른 종목 신호로 LIVE 를 오표시하지 않는다.
  const isLive = demoActive
    ? true
    : activeCallId != null
      ...
      >
        ...
        <section className="card p-0 flex flex-col overflow-hidden min-h-0">
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            {/* 시연 모드에서는 팩트체크 pane(55%), 실데이터 연동 시에는 기존 AI 점수 차트. */}
            {demoActive ? (
              <div style={{ flex: '55 1 0%' }} className="flex flex-col min-h-0 overflow-hidden">
                <FactCheckPanel key={loopKey} items={factCheckItems} sttStep={sttStep} />
              </div>
            ) : (
