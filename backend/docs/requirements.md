...
- **수량 결정 책임 분리:** 백엔드는 수량을 직접 계산하지 않고 사용자의 `buyAmountRatio`만 `order_ratio`로 전달한다. 최종 수량은 Trading Terminal이 로컬 잔고·현재가 기준으로 산출하며, 이는 "중앙 서버의 수량 결정"을 배제해 미등록 투자일임업 리스크를 회피하기 위한 설계다. `Trade.orderQty`는 PENDING 시점엔 0 센티널이며, 체결 콜백의 `executed_qty`로 덮어쓴다.
