...
- **통신 방식:** HTTP POST (저빈도 배치, 하루 1~2회)
- **엔드포인트:** `POST /api/v1/internal/earnings-calendar`
- **인증:** 내부 서비스 전용 (서버 간 통신, `X-Internal-Secret` 헤더로 공유 시크릿 검증)
- **설명:** 관심 종목의 다음 어닝콜 예정일 및 확정 여부를 백엔드 DB에 적재합니다. 백엔드는 해당 데이터를 캐싱하여 프론트엔드 `/api/v1/earnings-calendar` GET 요청에 응답합니다.

| 필드명 | 타입 | 필수 | 설명 |
| :--- | :--- | :---: | :--- |
| `earnings` | Array | Y | 어닝콜 일정 항목 배열 |
| `earnings[].ticker` | String | Y | 종목 심볼 (예: `NVDA`) |
| `earnings[].company_name` | String | Y | 회사명 (예: `NVIDIA Corporation`) |
| `earnings[].scheduled_at` | Long | Y | 예정 어닝콜 시각 (Unix Epoch Second, UTC) |
| `earnings[].is_confirmed` | Boolean | Y | 일정 확정 여부 (`false`이면 예상치) |
...
### 7.5. 관심종목 (Watchlist) — 구현 예정

| Method | Endpoint | 인증 | 설명 |
| :--- | :--- | :---: | :--- |
| GET | `/api/v1/watchlist` | 필요 | 내 관심종목 목록 조회. 응답: `[{ticker, company_name, price, change_pct, next_earnings_at, is_live}]` |
| POST | `/api/v1/watchlist` | 필요 | 관심종목 추가. 요청: `{ticker}` |
| DELETE | `/api/v1/watchlist/{ticker}` | 필요 | 관심종목 삭제 |
| GET | `/api/v1/watchlist/search?q={query}` | 필요 | 종목 검색 (심볼·회사명). Data Pipeline 제공 데이터 기반 |

### 7.6. 어닝콜 일정 (Earnings Calendar) — 구현 예정

| Method | Endpoint | 인증 | 설명 |
| :--- | :--- | :---: | :--- |
| GET | `/api/v1/earnings-calendar?tickers=NVDA,AAPL` | 필요 | 관심종목 어닝콜 일정 조회. 응답: `[{ticker, company_name, scheduled_at, is_confirmed}]` |
