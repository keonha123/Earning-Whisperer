...
| 증상 | 원인 |
|---|---|
| 창은 뜨는데 데이터가 안 들어온다 | `BACKEND_URL` 이 비어 있거나 오타입니다. 값을 바꾼 뒤 앱을 재시작하지 않은 경우도 많습니다 |
| 로그인 후 앱으로 돌아오지 않는다 | 9000 포트가 다른 프로그램에 점유되어 있습니다 |
| 모듈을 찾지 못한다는 오류 | WSL · Windows 터미널 혼용 문제입니다 ([3-1](#3-1-받아오기)) |
| health 가 열리지 않는다 | 인스턴스가 중지 상태일 수 있습니다 ([4장](#4-일상-운영)) |
...
**인스턴스 시작 · 정지와 SSH 접속은 서로 다른 권한입니다.** 둘을 따로 부여하게 됩니다.

| 하려는 것 | 필요한 것 | 인스턴스가 꺼져 있을 때 |
|---|---|---|
...
AWS 는 서버가 존재하는 시간에 과금합니다. 그래서 이 인스턴스는 필요할 때만 켜는 것을 원칙으로 합니다.
...
- **Gemini 임베딩 무료 등급은 하루 1,000요청이고 한국시간 16:00 에 리셋됩니다.** 근거 적재가 이 한도를 쓰므로 [6장](#6-근거-데이터-qdrant)의 스냅샷 절차를 따르는 편이 안전합니다.
